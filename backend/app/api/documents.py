...
BASE_DIR = Path("/var/lib/ai-vulnerability-management/documents")
...
def safe_name(filename):
    name = Path(filename or "supporting_document").name
...
async def upload_document(
    ...
    document_id = "DOC-" + hashlib.sha1(f"{original_name}:{now()}".encode()).hexdigest()[:12].upper()
...
@router.put("/{document_id}/mappings")
def update_document_mappings(document_id: str, payload: dict):
    records = load_db()
