...
async def upload_evidence(
    ...
    ev = Evidence(evidence_id=evidence_id, finding_id=finding_id, asset_id=asset_id, control_id=control_id, framework=framework, filename=safe_name, file_path=str(target_path), source=source, description=description)
    db.add(ev)
    db.commit()
    db.refresh(ev)
    return ev
